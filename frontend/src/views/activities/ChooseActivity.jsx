...
export default function ChooseActivity({ activity, onComplete, onEmptyCheck }) {
      ...
      <div className="w-full flex flex-col items-center justify-center mt-10 mb-8 px-4">
        <div className="max-w-xl w-full text-center">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#1137a0] drop-shadow-sm leading-snug">
            {activity?.prompt ||
              (lang === "tl" ? "Piliin ang tamang sagot." : "Choose the correct answer.")}
          </h2>
        </div>
      </div>

      {/* CHOICES (2x2 grid) */}
      <div className="grid grid-cols-2 gap-6 sm:gap-8 md:gap-10 justify-items-center max-w-3xl mx-auto pb-10 px-4">
        {choices.map((choice) => (
          <ActivityCard
            key={choice.key}
            image={choice.image}
        ...
        {showHint && (
          ...
          >
            ...
            {lang === "tl"
              ...
              : "Look carefully at the images and choose the correct one."}
          </motion.div>
        )}
      </AnimatePresence>
